...
def mobilenet_v2_net(inputs, is_training = False):
    conv_def = globals()["V2_DEF_" + config.model_name] #v5
    with tf.contrib.slim.arg_scope(mobilenet_v2.training_scope()):
        logits, end_points = mobilenet_v2.mobilenet_base(inputs, num_classes=0, 
            conv_defs = conv_def, is_training = is_training)

        if conv_def == V2_DEF_v4 :
            layers = [4,8,12,16,17]
            
        elif conv_def == V2_DEF_v5 :
            layers = [6,10,14,17,19]
            
        elif conv_def == V2_DEF_v5s :
            layers = [4,6,8,9,10]
            
        elif conv_def == V2_DEF_pixel :
            layers = [6,10,14,16]

        elif conv_def == V2_DEF_v6 :
